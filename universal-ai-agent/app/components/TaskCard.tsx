  ...
  return (
    <div className="border rounded-lg p-4 bg-white shadow-sm hover:shadow-md transition-shadow">
      <div className="flex justify-between items-start mb-2">
        <h3 className="font-semibold text-lg truncate">{task.goal}</h3>
      ...
      <div className="text-sm text-gray-600 space-y-1">
        ...
        {task.totalDuration && <p>Duration: {task.totalDuration}ms</p>}
        {task.totalCost && <p>Cost: ${task.totalCost.toFixed(4)}</p>}
      </div>
      ...
      {isExpanded && (
        <div className="mt-4 p-3 bg-gray-50 rounded">
          ...
          <div className="text-sm space-y-1">
            ...
            {task.totalDuration && <p><strong>Processing Time:</strong> {task.totalDuration}ms</p>}
            {task.totalCost && <p><strong>Estimated Cost:</strong> ${task.totalCost.toFixed(4)}</p>}
          </div>
        </div>
      )}
    </div>
